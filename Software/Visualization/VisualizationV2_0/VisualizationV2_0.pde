...
Visualization V2.0
 ...
 -change colors of the temp int the Btemp with map
 */
...
void tempB(float temp) { // body temperature between 33 a 41
  // color definitions
 println(temp);
 //deixar mais fliudo de acordo com rgb
float G = map(temp,0,40,0,255);
float R = map(temp,30,40,0,255);
float B = map(temp,40,0,0,255);
fill(R,G,B);
box(100);
}
